...
interface GitHubApiOptions {
	tokens: string[];
	apiUrl?: string;
	onRateLimitChange?: (token: string, rateLimit: RateLimit) => void;
}
...
export class GitHubApi {
	...
	async #getToken(): Promise<string> {
		const token = this.#tokens
			.sort((a, b) => {
				const rateLimitA = this.#rateLimits[a];
				const rateLimitB = this.#rateLimits[b];
				if (!rateLimitA) return -1;
				if (!rateLimitB) return 1;
				return (rateLimitB.remaining ?? 0) - (rateLimitA.remaining ?? 0);
			})
			.find((token) => {
				const rateLimit = this.#rateLimits[token];
				if (!rateLimit) return true;
				if ((rateLimit.remaining ?? 0) > 0) return true;
				return (rateLimit.resetTimestamp ?? 0) < Date.now();
			});
		if (!token) {
			// No tokens are available, wait for the earliest reset
			const [firstAvailableToken, rateLimit] = Object.entries(
				this.#rateLimits,
			).sort(
				(a, b) => (a[1].resetTimestamp ?? 0) - (b[1].resetTimestamp ?? 0),
			)[0];
			...
			return new Promise((resolve) => {
				setTimeout(
					() => {
						resolve(firstAvailableToken);
					},
					(rateLimit.resetTimestamp ?? 0) -
						Date.now() +
			...
			const req = request(
				...
				(res) => {
					...
					res.on("end", () => {
						...
						this.#rateLimits[token] = rateLimit;
						this.#onRateLimitChange(token, rateLimit);
						if (!json.data) {
							if (json.errors) {
								Logger.error("[query]", "<=", json.errors);
								return reject({ error: json.errors, rateLimit, token });
							}
